...
class Collect
{
	...
	public function xml_db($video,$localId)
	{
		...
		$v_data['v_tags'] = $video->keywords;//影片关键词
		$v_data['v_nickname'] = $video->nickname;//影片别名
		$v_data['v_reweek'] = $video->reweek;//影片更新周期
		$v_data['v_douban'] = $video->douban;//影片豆瓣评分
		$v_data['v_mtime'] = $video->mtime;//影片时光网评分
		$v_data['v_imdb'] = $video->imdb;//影片imdb评分
		$v_data['v_tvs'] = $video->tvs;//影片上映电视台
		$v_data['v_company'] = $video->company;//影片发行公司
		$v_data['v_ver'] = $video->ver;//影片版本
		$v_data['v_longtxt'] = $video->longtxt;//影片备用备注信息
		...
		$v_data['v_len'] = $video->len;//影片时长
		$v_data['v_total'] = $video->total;//影片集数
...
//		}
		...
		if(is_array($rs))
		{
			...
			//{
				if($v_data['v_downdata']==$rs['v_downdata']&&$v_data['v_playdata']==$rs['v_playdata'])
				{
					return $autocol_str.'数据<font color=red>'.$v_data['v_name'].'</font>地址无变化,无需更新<br>';
				}
				//if 勾选[只更新影片地址]
...
//		}
	...
	function update_pic($v_data,$rs)
	{
		...
		if(strpos($rs['v_pic'],'uploads/')===0||($app_ftp==1 && strpos($rs['v_pic'],$app_ftpurl)!==false))
		{
			unset($v_data['v_pic']);
		}
		else
