...
function makePageNumber($currentPage,$pagelistLen,$totalPages,$linkType,$currentTypeId=0,$vid=0){
	...
	for($pagenumber=$beginPage;$pagenumber<=$endPage;$pagenumber++){
		...
		if($pagenumber==$currentPage){
			if ($linkType=="search" || $linkType=="channel" || $linkType=="comment" || $linkType=="topicpage"|| $linkType=="cascade"|| $linkType=="gbook"||$linkType=="newssearch"||$linkType=="newspage"){
			...
			break;
			case "videolist":
			global $order,$type,$keyword,$v_state,$v_commend,$repeat,$topic,$playfrom,$etype,$empty,$rlen,$allrepeat,$v_recycled,$v_isunion,$jqtype,$area,$year,$yuyan,$letter,$commend,$ver,$tid;
				$strPageNumber.="<a href='?page=".$pagenumber."&order=".$order."&type=".$type."&etype=".$etype."&keyword=".$keyword."&v_state=".$v_state."&v_commend=".$v_commend."&repeat=".$repeat."&allrepeat=".$allrepeat."&topic=".$topic."&playfrom=".$playfrom."&empty=".$empty."&rlen=".$rlen."&v_recycled=".$v_recycled."&v_isunion=".$v_isunion."&jqtype=".$jqtype."&area=".$area."&year=".$year."&yuyan=".$yuyan."&letter=".$letter."&commend=".$commend."&ver=".$ver."&tid=".$tid."'>".$pagenumber."</a>";
			break;
			case "collectfilters":
