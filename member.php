...
elseif($action=='cz')
{
	$key=mysql_real_escape_string($_POST['cckkey']);
...
elseif($action=='cc')
{
	$ccgid=$_SESSION['sea_user_group'];
	$ccuid=$_SESSION['sea_user_id'];
...
EOT;
                            echo  "<label style=\"color:#aaa\">".
								  "用户级别：</label><span class=\"orange\">{$ccgroup}</span>";
									$sql="select * from sea_member_group";
							$dsql->SetQuery($sql);
							$dsql->Execute('al');
...
EOT;
							...
							while($rowr=$dsql->GetObject('al'))
							{
								echo "<input type=\"submit\" style=\"background:#B17354;width: 100px; height: 25px;border:0;color:#fff;font-size:14px;margin:0 0 0 10px;\" value='".$rowr->gname."' onClick=\"self.location='?action=hyz&gid=".$rowr->gid."';\"></span>";
							}
								echo "<div class=\"row\">".
                                 "<label style=\"color:#aaa\">".
                                    "可用积分：</label>{$ccjifen}</div>".
