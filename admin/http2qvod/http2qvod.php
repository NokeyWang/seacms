...
{
	$pagesize = 30;
	$sql="select count(*) as dd from sea_playdata where body like '%qvod%' and body like '%http://%'";
	$row = $dsql->GetOne($sql);
	$num = $row['dd'];
	if ($num%$pagesize) {
	...
	$sql = "select v_id,body from `sea_playdata` where body like '%qvod%' and body like '%http://%' order by v_id ASC limit $page $pagesize";
	$dsql->SetQuery($sql);
...
//			$httpurla = explode(httpurl,"/");
		...
		if($isChange){
			$dsql->ExecNoneQuery("update sea_playdata set body='".$tempdata."' where v_id=".$row['v_id']);
			echo "<font color='green'>更新数据成功</font><br/>";
		}else{
			if($delmode==1){
				$dsql->ExecNoneQuery("delete from sea_playdata where v_id=".$row['v_id']);
...
<br/>
<div align=center>Copyright 2011-2012 All rights reserved. <a href="http://www.seacms.net/" target="_blank">seacms</a>
</div>
</div>
</body>
</html>